...
async def test_telegram_gateway_message_routing() -> None:
    ...
    with patch("f.telegram_gateway.main.get_variable", return_value="fake-token"), \
         patch("f.telegram_gateway._gateway_logic.ClientRepository.ensure_registered", AsyncMock(return_value=(None, None))):
