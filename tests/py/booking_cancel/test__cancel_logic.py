...
from f.booking_cancel._cancel_booking_logic import (
    CancelBookingError,
    authorize_actor,
...
async def test_authorize_actor_client_success() -> None:
    ...
    # Should not raise
    authorize_actor(input_data, booking)  # type: ignore
...
async def test_authorize_actor_mismatch() -> None:
    ...
    with pytest.raises(CancelBookingError, match="unauthorized"):
        authorize_actor(input_data, booking)  # type: ignore
...
async def test_execute_cancel_booking_success() -> None:
    ...
    res = await execute_cancel_booking(repo, input_data, booking)  # type: ignore
...
async def test_execute_cancel_booking_already_cancelled() -> None:
    ...
    with pytest.raises(CancelBookingError, match="already_cancelled"):
        await execute_cancel_booking(repo, input_data, {})  # type: ignore
