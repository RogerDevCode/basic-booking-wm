...
async def test_ai_agent_llm_success() -> None:
    ...
    mock_llm_res.content = '{"intent": "crear_cita", "confidence": 0.9, "entities": {"date": "mañana"}, "needs_more": false, "follow_up": null}'
    mock_llm_res.provider = "openai"
    
    # Patch in the right place
    with patch("f.internal.ai_agent._llm_client.get_variable", return_value="openai"), \
