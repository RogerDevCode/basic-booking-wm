...
async def test_patient_profile_get_success() -> None:
    ...
    mock_db.fetch.side_effect = [
        ...
        [{"client_id": "c1", "name": "Test User", "email": "t@t.com", "phone": "123", "telegram_chat_id": None, "timezone": "UTC", "gcal_calendar_id": None}] # find_or_create_client
    ]
    
    async def mock_with_tenant(db, tid, op):
        return await op()

    with patch("f.web_patient_profile.main.create_db_client", return_value=mock_db), \
