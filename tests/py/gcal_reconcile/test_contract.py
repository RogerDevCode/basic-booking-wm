...
from f.gcal_reconcile._reconcile_models import ReconcileResult
...
async def test_gcal_reconcile_success() -> None:
    ...
    with patch("f.gcal_reconcile.main.create_db_client", return_value=mock_db), \
         patch("f.gcal_reconcile.main.with_tenant_context", side_effect=mock_with_tenant), \
         patch("f.gcal_reconcile.main.sync_booking_to_gcal", AsyncMock(return_value={
             "providerEventId": "new-p-123",
             "clientEventId": None,
             "errors": []
         })):
