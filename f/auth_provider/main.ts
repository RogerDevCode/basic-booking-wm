...
async function adminGenerateTempPassword(sql: postgres.Sql, providerId: string): Promise<Result<TempPasswordResult>> {
  try {
    // Verify provider exists
    const providers = await sql`SELECT id, name, email FROM providers WHERE id = ${providerId}::uuid LIMIT 1`;
    ...
    return [new Error(`admin_generate_temp_failed: ${msg}`), null];
  }
}
...
async function providerChangePassword(
  ...
  try {
    ...
    const providers = await sql`SELECT id, name, password_hash FROM providers WHERE id = ${providerId}::uuid LIMIT 1`;
    ...
    const msg = e instanceof Error ? e.message : String(e);
    return [new Error(`provider_change_failed: ${msg}`), null];
  }
}
...
async function providerVerify(
  ...
  try {
    ...
    const msg = e instanceof Error ? e.message : String(e);
    return [new Error(`provider_verify_failed: ${msg}`), null];
  }
}
...
export async function main(rawInput: unknown): Promise<{
  ...
  try {
    if (input.action === 'admin_generate_temp') {
      const [err, result] = await adminGenerateTempPassword(sql, input.provider_id);
    ...
    if (input.action === 'provider_change') {
      ...
      if (currentPassword == null || newPassword == null) {
        return { success: false, data: null, error_message: 'provider_change requires current_password and new_password' };
      }
      const [err, result] = await providerChangePassword(sql, input.provider_id, currentPassword, newPassword);
      if (err != null) return { success: false, data: null, error_message: err.message };
      return { success: true, data: result, error_message: null };
    }
    ...
    if (input.action === 'provider_verify') {
      ...
      if (password == null) {
        return { success: false, data: null, error_message: 'provider_verify requires current_password' };
      }
      const [err, result] = await providerVerify(sql, input.provider_id, password);
      if (err != null) return { success: false, data: null, error_message: err.message };
      return { success: true, data: result, error_message: null };
    }
