...
from ._change_role_models import InputSchema, ChangeRoleResult

MODULE = "web_auth_change_role"

async def main(args: dict[str, Any]) -> Result[ChangeRoleResult]:
    # 1. Validate Input
    try:
    ...
    try:
        ...
        async def operation() -> Result[ChangeRoleResult]:
            ...
            target_rows = await conn.fetch(
                "SELECT user_id, full_name, role FROM users WHERE user_id = $1::uuid LIMIT 1",
            ...
            update_rows = await conn.fetch(
                ...
                """,
                input_data.new_role, input_data.target_user_id
            )
