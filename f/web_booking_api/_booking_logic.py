...
class BookingRepository:
    ...
    async def resolve_tenant_for_booking(self, booking_id: str) -> Result[str]:
        ...
        if not rows:
            return fail("cita_no_encontrada")
    ...
    async def resolve_client_id(self, user_id: str) -> Result[str]:
        ...
        if not rows or not rows[0].get("email"):
            return fail("cliente_no_registrado")
    ...
    async def lock_provider(self, provider_id: str) -> Result[bool]:
        ...
        if not rows:
            return fail("proveedor_inactivo")
        return ok(True)
    ...
    async def insert_booking(self, data: dict[str, Any]) -> Result[dict[str, Any]]:
        ...
        if not rows:
            return fail("error_insercion_booking")
    ...
    async def update_status(self, booking_id: str, status: str, reason: str | None = None) -> Result[bool]:
        rows = await self.db.fetch(
        ...
        if not rows:
            return fail("error_actualizacion_booking")
        return ok(True)
