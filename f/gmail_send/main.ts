...
function buildEmailContent(
  ...
  switch (messageType) {
    case 'booking_created':
      ...
      color = '#4CAF50';
      body = `
        <h2 style="color: ${color};">Cita Agendada Exitosamente</h2>
    ...
    case 'booking_confirmed':
      ...
      color = '#4CAF50';
      body = `
        <h2 style="color: ${color};">Tu Cita Ha Sido Confirmada</h2>
        <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
          <tr><td style="padding: 8px 0; font-weight: bold;">📅 Fecha:</td><td>${date}</td></tr>
          <tr><td style="padding: 8px 0; font-weight: bold;">🕐 Hora:</td><td>${time}</td></tr>
          <tr><td style="padding: 8px 0; font-weight: bold;">👨‍⚕️ Doctor:</td><td>${providerName}</td></tr>
    ...
    case 'booking_cancelled':
      ...
      color = '#F44336';
      body = `
        <h2 style="color: ${color};">Cita Cancelada</h2>
    ...
    case 'booking_rescheduled':
      ...
      color = '#FF9800';
      body = `
        <h2 style="color: ${color};">Cita Reprogramada</h2>
    ...
    case 'reminder_24h':
      ...
      color = '#2196F3';
      body = `
        <h2 style="color: ${color};">Recordatorio de Cita</h2>
    ...
    case 'reminder_2h':
      ...
      color = '#FF9800';
      body = `
        <h2 style="color: ${color};">Tu Cita es Pronto</h2>
    ...
    case 'reminder_30min':
      ...
      color = '#F44336';
      body = `
        <h2 style="color: ${color};">¡Tu Cita es en 30 Minutos!</h2>
    ...
    case 'no_show':
      ...
      color = '#9E9E9E';
      body = `
        <h2 style="color: ${color};">No Asististe a tu Cita</h2>
    ...
    case 'provider_schedule_change':
      ...
      color = '#9C27B0';
      body = `
        <h2 style="color: ${color};">Cambio de Horario</h2>
...
async function sendWithRetry(
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (err.message.includes('5') && !err.message.includes('5.7') && !err.message.includes('429')) {
        // Transient SMTP error, retry
      } else if (err.message.includes('4')) {
        // Permanent error, no retry
        return { sent: false, message_id: null, error: `Permanent error: ${err.message}` };
      }
    }
...
export async function main(rawInput: unknown): Promise<{ success: boolean; data: GmailSendData | null; error_message: string | null }> {
  try {
    ...
    const { recipient_email, message_type, booking_details, action_links } = parsed.data;
    ...
    if (!smtpUser || !smtpPass) {
      return { success: false, data: null, error_message: 'SMTP credentials not configured (GMAIL_USER/GMAIL_PASSWORD)' };
    }
    ...
    const { subject, html } = buildEmailContent(message_type, booking_details, action_links);
