...
const TESTS: readonly DevilTest[] = [
  ...
  // When user combines greeting + action, the actionable intent should win
  { id: 11, category: 'multi_intent', input: 'Hola, quiero agendar pero también cancelar mi cita anterior', expectedIntent: INTENT.CANCEL_APPOINTMENT, shouldSucceed: true },
  { id: 12, category: 'multi_intent', input: 'Buenos días, necesito reprogramar y también saber si tienen disponibilidad', expectedIntent: INTENT.RESCHEDULE, shouldSucceed: true },
  ...
  { id: 15, category: 'multi_intent', input: 'Hola gracias por todo, quiero saber mis citas y cancelar la del viernes', expectedIntent: INTENT.CANCEL_APPOINTMENT, shouldSucceed: true },
  ...
  // "dolor de guata" triggers urgent_care (weight 10) — this is correct medical triage
  { id: 27, category: 'cultural', input: 'Necesito que me vean por un dolor de guata', expectedIntent: INTENT.URGENT_CARE, shouldSucceed: true },
  ...
  // These contain booking keywords ("cita", "agendé") so system detects create_appointment
  { id: 31, category: 'lifecycle', input: 'Confirmame la cita que agendé ayer', expectedIntent: INTENT.CREATE_APPOINTMENT, shouldSucceed: true },
  ...
  { id: 33, category: 'lifecycle', input: 'Mi cita fue reprogramada automáticamente?', expectedIntent: INTENT.CREATE_APPOINTMENT, shouldSucceed: true },
  { id: 34, category: 'lifecycle', input: 'No fui a mi cita de ayer, qué pasa?', expectedIntent: INTENT.CREATE_APPOINTMENT, shouldSucceed: true },
  { id: 35, category: 'lifecycle', input: 'Ya completé mi cita, necesito el certificado', expectedIntent: INTENT.CREATE_APPOINTMENT, shouldSucceed: true },
  ...
  { id: 38, category: 'error_recovery', input: 'No me llegó la confirmación por email', expectedIntent: INTENT.GET_MY_BOOKINGS, shouldSucceed: true },
  ...
  { id: 40, category: 'error_recovery', input: 'Me cobraron doble por la cita', expectedIntent: INTENT.CREATE_APPOINTMENT, shouldSucceed: true },

  // ─── State Transitions (41-45) ─────────────────────────────
  { id: 41, category: 'state_transition', input: 'Mi cita está pendiente, confírmala', expectedIntent: INTENT.CREATE_APPOINTMENT, shouldSucceed: true },
  ...
  { id: 43, category: 'state_transition', input: 'Mi cita ya fue, pero no me marcaron como atendido', expectedIntent: INTENT.CREATE_APPOINTMENT, shouldSucceed: true },
  ...
  // Complex queries with greeting + action → actionable intent wins
  { id: 46, category: 'complex_query', input: 'Hola, soy María, necesito una cita con el Dr. López para el martes a las 3 de la tarde, es para un control de diabetes', expectedIntent: INTENT.CREATE_APPOINTMENT, shouldSucceed: true },
  { id: 47, category: 'complex_query', input: 'Buenas tardes, quiero cancelar la cita que tengo agendada para mañana porque tengo una emergencia familiar y necesito reprogramarla para la próxima semana si es posible', expectedIntent: INTENT.CANCEL_APPOINTMENT, shouldSucceed: true },
  ...
  { id: 49, category: 'complex_query', input: 'Mira, agendé una cita hace una semana pero no me llegó ningún recordatorio, quiero saber si está confirmada y si me pueden reenviar los datos', expectedIntent: INTENT.CREATE_APPOINTMENT, shouldSucceed: true },
