...
from ._constants import (
    INTENT, CONFIDENCE_THRESHOLDS, INTENT_KEYWORDS, OFF_TOPIC_PATTERNS,
    THANK_YOU_WORDS, URGENCY_WORDS, FLEXIBILITY_KEYWORDS, DAY_NAMES,
    RELATIVE_DATES, SERVICE_TYPES, GREETINGS, GREETING_PHRASES,
    FAREWELLS, FAREWELL_PHRASES, ESCALATION_THRESHOLDS,
    RULE_CONFIDENCE_VALUES, SOCIAL_CONFIDENCE_VALUES, CONFIDENCE_BOUNDARIES
)
from ._ai_agent_models import (
    AIAgentInput, ConversationState, EntityMap, AvailabilityContext,
    IntentResult, ContextAdjustment, EscalationLevel
)
...
def adjust_intent_with_context(
    ...
    if state.active_flow == 'selecting_datetime' and re.match(r"^\d", lower):
        return {
            ...
            "intent": INTENT['CREAR_CITA'],
            "confidence": 0.90,
            "reason": f"Context: user provided date/time in datetime selection flow"
        }
...
def extract_entities(text: str) -> EntityMap:
    ...
    data: Dict[str, Optional[str]] = {
        "date": None, "time": None, "provider_name": None, "provider_id": None,
        "service_type": None, "service_id": None, "booking_id": None,
        "channel": None, "reminder_window": None
    }
...
def detect_context(text: str, entities: EntityMap) -> AvailabilityContext:
    ...
    if any(x in lower for x in ['mañana', 'manana']): time_pref = 'morning'
    elif 'tarde' in lower: time_pref = 'afternoon'
    elif 'noche' in lower: time_pref = 'evening'
...
def determine_escalation_level(intent: str, text: str, confidence: float) -> EscalationLevel:
    ...
    if intent == INTENT['URGENCIA'] and confidence >= ESCALATION_THRESHOLDS['medical_emergency_min']:
        patterns = r"muerte|morir|no respiro|infarto|desmay|sangr|convul|paro|dolor.*pecho|dificultad.*respir|no puedo.*respir"
    ...
    if confidence < ESCALATION_THRESHOLDS['human_handoff_max'] and intent not in [INTENT['SALUDO'], INTENT['DESPEDIDA'], INTENT['AGRADECIMIENTO']]:
...
def generate_ai_response(
    ...
    if intent == INTENT['SALUDO']:
        return "Hola, soy tu asistente médico. ¿En qué puedo ayudarte?", True, "¿Deseas agendar, cancelar o cambiar una cita?"
...
def detect_social(text: str) -> Optional[Tuple[str, float]]:
    lower = text.lower().strip()
    if lower in GREETINGS: return (INTENT['SALUDO'], SOCIAL_CONFIDENCE_VALUES['greeting_exact'])
    if any(p in lower for p in GREETING_PHRASES): return (INTENT['SALUDO'], SOCIAL_CONFIDENCE_VALUES['greeting_phrase'])
    if lower in FAREWELLS: return (INTENT['DESPEDIDA'], SOCIAL_CONFIDENCE_VALUES['farewell_exact'])
    if any(p in lower for p in FAREWELL_PHRASES): return (INTENT['DESPEDIDA'], SOCIAL_CONFIDENCE_VALUES['farewell_phrase'])
    return None
