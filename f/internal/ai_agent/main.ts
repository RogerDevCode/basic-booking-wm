...
function detectContext(text: string, entities: EntityMap): AvailabilityContext {
  ...
  if (lower.includes('mañana') && !is_tomorrow) time_preference = 'morning';
  else if (lower.includes('tarde')) time_preference = 'afternoon';
  ...
  return {
    ...
    is_urgent,
    is_flexible,
    is_specific_date: entities.date != null && !is_today && !is_tomorrow,
    time_preference,
    day_preference,
  };
}
...
function generateAIResponse(
  ...
  if (intent === INTENT.URGENT_CARE) {
    return {
      aiResponse: "Entiendo que es una situación urgente. He localizado 2 espacios prioritarios para hoy mismo.",
      needsMoreInfo: false,
  ...
  if (responseType === 'clarifying_question') {
    return {
      aiResponse: "Con gusto te ayudo a agendar.",
      needsMoreInfo: true,
