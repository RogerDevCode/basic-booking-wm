...
GuardrailResult = Union[GuardrailPass, GuardrailBlocked]
...
LEAKAGE_PATTERNS = [
    'SYSTEM_INSTRUCTIONS', 'UNTRUSTED INPUT', 'BEGIN USER DATA',
    'END USER DATA', 'INTENT DEFINITIONS', 'REGLAS DE DESEMPATE'
]
...
def verify_urgency(result: IntentResult, text: str) -> IntentResult:
    ...
    for k, v in accents.items(): lower = lower.replace(k, v)
