...
from ._scheduling_models import (
    AvailabilityQuery, AvailabilityResult, TimeSlot, 
    ScheduleOverrideRow, ProviderScheduleRow, BookingTimeRow, ServiceRow,
    OverrideValidation, AffectedBooking
)
...
def generate_slots_for_rule(
    ...
    while current_min + slot_duration_min <= end_min:
        ...
        is_booked = any(
            slot_start_ts < b_end and slot_end_ts > b_start
...
async def get_availability(
    ...
    try:
        ...
        special_override = next((o for o in overrides if not o["is_blocked"] and o["start_time"] and o["end_time"]), None)
...
async def get_availability_range(
    ...
    while iter_date <= end_dt:
        ...
        err, res = await get_availability(db, {
