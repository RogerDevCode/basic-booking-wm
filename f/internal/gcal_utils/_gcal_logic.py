...
def build_gcal_event(
    ...
    calendar_type: Literal['provider', 'client'] = 'provider'
) -> GoogleCalendarEvent:
    # calendar_type reserved for future per-audience event customization
    
    title = f"[CANCELLED] Cita Médica - {booking['provider_name']}" if booking['status'] == 'cancelled' \
    ...
    description_parts = [
        ...
        "",
        "Esta cita ha sido cancelada." if booking['status'] == 'cancelled' \
