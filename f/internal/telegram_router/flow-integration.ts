...
function buildMessagePayload(text: string, chatId = '12345', firstName = 'Roger'): Record<string, unknown> {
  return {
...
function buildCallbackPayload(data: string, chatId = '12345', messageId = 999): Record<string, unknown> {
  return {
    update_id: Math.floor(Math.random() * 1000000),
    callback_query: {
