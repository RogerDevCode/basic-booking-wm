...
async function getConvState(redis: Redis, chatId: string): Promise<ConvState> {
...
async function setConvState(redis: Redis, chatId: string, bookingState: BookingState, draft: DraftBooking, messageId: number | null): Promise<void> {
  await redis.set(`${CONV_PREFIX}${chatId}`, JSON.stringify({ bookingState, draft, messageId }), 'EX', 1800);
}

async function clearConvState(redis: Redis, chatId: string): Promise<void> {
