...
async def sync_booking_to_gcal(
    ...
    if booking["provider_calendar_id"]:
        ...
        async def sync_op() -> Result[Dict[str, Any]]:
    ...
    if booking["client_calendar_id"]:
        ...
        async def sync_op_cli() -> Result[Dict[str, Any]]:
    ...
    if booking["status"] == 'cancelled':
        if result["providerEventId"] and booking["provider_calendar_id"]:
            ...
            err_d, _ = await retry_with_backoff(lambda: call_gcal_api('DELETE', cal_id, f"events/{eid}"), max_retries)
            if not err_d:
        ...
        if result["clientEventId"] and booking["client_calendar_id"]:
            cal_id = booking["client_calendar_id"]
            eid = result["clientEventId"]
            err_d, _ = await retry_with_backoff(lambda: call_gcal_api('DELETE', cal_id, f"events/{eid}"), max_retries)
            if not err_d:
                result["clientEventId"] = None
            else:
                result["errors"].append(f"Client delete: {err_d}")
