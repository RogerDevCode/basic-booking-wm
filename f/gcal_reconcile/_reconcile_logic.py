...
async def retry_with_backoff[T](fn: Callable[[], Awaitable[Result[T]]], max_retries: int) -> Result[T]:
...
async def sync_booking_to_gcal(booking: BookingRow, max_retries: int) -> SyncResult:
    ...
    if booking["status"] == "cancelled":
        if result["providerEventId"] and booking["provider_calendar_id"]:
            ...
            err_d, _ = await retry_with_backoff(
                lambda: cast("Awaitable[Result[object]]", call_gcal_api("DELETE", cal_id_del, f"events/{eid}")),
                max_retries,
            )
            if not err_d:
        ...
        if result["clientEventId"] and booking["client_calendar_id"]:
            ...
            err_d_cli, _ = await retry_with_backoff(
                lambda: cast("Awaitable[Result[object]]", call_gcal_api("DELETE", cal_id_del_cli, f"events/{eid_cli}")),
                max_retries,
            )
            if not err_d_cli:
