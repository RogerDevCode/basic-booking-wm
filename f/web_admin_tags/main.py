...
from ._tags_models import InputSchema, CategoryRow, TagRow
from ._tags_logic import verify_admin_access, TagRepository
...
async def _main_async(args: dict[str, Any]) -> Result[Any]:
    ...
    try:
        ...
        async def operation() -> Result[Any]:
            ...
            if err_access: return fail(err_access)

            repo = TagRepository(conn)
            action = input_data.action
            
            if action == 'list_categories': return await repo.list_categories()
            elif action == 'create_category':
                if not input_data.name: return fail("REQUIRED: name")
                return await repo.create_category(input_data.name, input_data.description, input_data.sort_order or 0)
            elif action == 'update_category':
                if not input_data.category_id: return fail("REQUIRED: category_id")
            ...
            elif action == 'list_tags': return await repo.list_tags(input_data.category_id)
            elif action == 'create_tag':
                if not input_data.category_id or not input_data.name: return fail("REQUIRED: category_id, name")
                return await repo.create_tag(input_data.category_id, input_data.name, input_data.description, input_data.color or '#808080', input_data.sort_order or 0)
            elif action == 'update_tag':
                if not input_data.tag_id: return fail("REQUIRED: tag_id")
                ...
                if err_c: return fail(err_c)
                err_t, tags = await repo.list_tags()
                if err_t: return fail(err_t)
