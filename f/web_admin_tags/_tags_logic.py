...
from ._tags_models import CategoryRow, TagRow, InputSchema
...
async def verify_admin_access(db: DBClient, user_id: str) -> Result[bool]:
    ...
    if not rows: return fail("UNAUTHORIZED: Admin user not found or inactive")
    if rows[0]["role"] != "admin": return fail("FORBIDDEN: Admin access required")
    return ok(True)
...
class TagRepository:
    ...
    async def create_category(self, name: str, description: Optional[str], sort_order: int) -> Result[CategoryRow]:
        try:
            rows = await self.db.fetch(
                "INSERT INTO tag_categories (name, description, sort_order) VALUES ($1, $2, $3) RETURNING *, 0 as tag_count",
    ...
    async def update_category(self, category_id: str, input_data: InputSchema) -> Result[CategoryRow]:
        try:
            ...
            query = f"UPDATE tag_categories SET {', '.join(fields)}, updated_at = NOW() WHERE category_id = ${idx}::uuid RETURNING *, 0 as tag_count"
            rows = await self.db.fetch(query, *params)
            if not rows: return fail("update_failed: not found")
            return ok(map_category(rows[0]))
        except Exception as e:
    ...
    async def set_category_status(self, category_id: str, active: bool) -> Result[Dict[str, Any]]:
        try:
            rows = await self.db.fetch(
                "UPDATE tag_categories SET is_active = $1, updated_at = NOW() WHERE category_id = $2::uuid RETURNING category_id, is_active",
                active, category_id
            )
            if not rows: return fail("not_found")
    ...
    async def list_tags(self, category_id: Optional[str] = None) -> Result[List[TagRow]]:
        try:
            if category_id:
                rows = await self.db.fetch(
                    """
    ...
    async def create_tag(self, category_id: str, name: str, description: Optional[str], color: str, sort_order: int) -> Result[TagRow]:
        try:
            rows = await self.db.fetch(
                """
                ...
                """,
                category_id, name, description, color, sort_order
            )
            if not rows: return fail("create_tag_failed")
    ...
    async def update_tag(self, tag_id: str, input_data: InputSchema) -> Result[TagRow]:
        try:
            ...
            if not fields: return fail("update_failed: no fields")
            params.append(tag_id)
            query = f"UPDATE tags SET {', '.join(fields)}, updated_at = NOW() WHERE tag_id = ${idx}::uuid RETURNING *, (SELECT name FROM tag_categories WHERE category_id = tags.category_id) as category_name"
            rows = await self.db.fetch(query, *params)
            if not rows: return fail("not_found")
            return ok(map_tag(rows[0]))
        except Exception as e:
    ...
    async def set_tag_status(self, tag_id: str, active: bool) -> Result[Dict[str, Any]]:
        try:
            rows = await self.db.fetch(
                "UPDATE tags SET is_active = $1, updated_at = NOW() WHERE tag_id = $2::uuid RETURNING tag_id, is_active",
                active, tag_id
            )
            if not rows: return fail("not_found")
