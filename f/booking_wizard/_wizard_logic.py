...
class DateUtils:
    ...
    def get_week_dates(offset: int) -> List[Dict[str, str]]:
        ...
        for i in range(7):
            ...
            dates.append({
                "date": d.isoformat(),
                "label": f"{d.day} {months_es[d.month-1]}",
                "dayName": days_es[d.weekday()]
            })
        return dates
...
class WizardRepository:
    ...
    async def get_names(self, provider_id: str, service_id: str) -> Result[Dict[str, str]]:
        ...
        if not p or not s: return fail("integrity_error")
        return ok({"provider": str(p[0]["name"]), "service": str(s[0]["name"])})

    async def create_booking(self, client_id: str, provider_id: str, service_id: str, date_str: str, time_str: str, tz: str, duration_min: int) -> Result[str]:
        ...
        try:
            rows = await self.db.fetch(
                ...
                """,
                client_id, provider_id, service_id, local_ts, tz, duration_min, ik
            )
            if not rows: return fail("insert_failed")
            ...
            await self.db.execute(
                ...
                VALUES ($1::uuid, null, 'confirmed', 'client', $2::uuid, 'Booking created via wizard', '{"channel": "telegram"}'::jsonb)
                """,
                bid, client_id
            )
            return ok(bid)
