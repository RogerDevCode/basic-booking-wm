...
from ._specialty_models import SpecialtyRow, InputSchema
...
async def create_specialty(db: DBClient, input_data: InputSchema) -> Result[SpecialtyRow]:
    if not input_data.name: return fail("create_failed: name is required")
    try:
        rows = await db.fetch(
            """
            ...
            input_data.category or 'Medicina', input_data.sort_order or 99
        )
...
async def update_specialty(db: DBClient, id: str, input_data: InputSchema) -> Result[SpecialtyRow]:
    try:
        ...
        if not rows: return fail(f"update_failed: specialty {id} not found")
        return ok(map_row(rows[0]))
    except Exception as e:
...
async def set_status(db: DBClient, id: str, active: bool) -> Result[Dict[str, Any]]:
    try:
        ...
        if "UPDATE 1" not in res: return fail(f"status_update_failed: specialty {id} not found")
