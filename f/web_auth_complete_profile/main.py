...
from ..internal._crypto import validate_password_policy, hash_password
from ..web_auth_register._register_logic import validate_rut
from ._complete_profile_models import InputSchema, CompleteProfileResult, UserRow

MODULE = "web_auth_complete_profile"

async def main(args: dict[str, Any]) -> Result[CompleteProfileResult]:
    # 1. Validate Input
    try:
    ...
    try:
        async def operation() -> Result[CompleteProfileResult]:
            ...
            existing_rows = await conn.fetch(
                ...
                LIMIT 1
                """,
                input_data.email, input_data.rut, user_id
            )
            if existing_rows:
            ...
            update_rows = await conn.fetch(
                ...
                """,
                input_data.rut, input_data.email, input_data.address, 
                input_data.phone, pwd_hash, input_data.timezone, user_id
            )
