...
async def resolve_client_id(db: DBClient, user_id: str, input_client_id: str | None) -> Result[str]:
    try:
        rows = await db.fetch(
            """
        ...
        if not cid:
            return fail("client_record_not_found")
        return ok(cid)
...
async def handle_join(db: DBClient, client_id: str, data: InputSchema) -> Result[WaitlistResult]:
    if not data.service_id:
        return fail("service_id_required")
    ...
    if rows:
        return fail("already_on_waitlist")
    ...
    return ok({"entries": [], "position": position, "message": f"Joined waitlist at position {position}"})


async def handle_leave(db: DBClient, client_id: str, waitlist_id: str | None) -> Result[WaitlistResult]:
    if not waitlist_id:
        return fail("waitlist_id_required")
...
async def handle_check_position(db: DBClient, client_id: str, waitlist_id: str | None) -> Result[WaitlistResult]:
    if not waitlist_id:
        return fail("waitlist_id_required")
    rows = await db.fetch(
