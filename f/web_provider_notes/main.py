...
from ._notes_models import InputSchema, NoteRow
        ...
        async def operation() -> Result[Any]:
            ...
            if action == 'create':
                ...
                return await repo.create(
                    input_data.provider_id, input_data.booking_id, input_data.client_id, 
                    input_data.content, input_data.tag_ids
                )
            elif action == 'read':
                if not input_data.note_id: return fail("read requires note_id")
                ...
                if notes_res[0]: return fail(notes_res[0])
                notes = notes_res[1] or []
                return ok({"notes": notes, "count": len(notes)})
            elif action == 'delete':
                if not input_data.note_id: return fail("delete requires note_id")
