...
async function getState(sql: postgres.Sql, serviceId: string): Promise<CircuitState | null> {
  ...
  return {
    ...
    state: String(row['state']) as 'closed' | 'open' | 'half-open',
    failure_count: Number(row['failure_count']),
    success_count: Number(row['success_count']),
    failure_threshold: Number(row['failure_threshold']),
    success_threshold: Number(row['success_threshold']),
    timeout_seconds: Number(row['timeout_seconds']),
    opened_at: typeof row['opened_at'] === 'string' ? row['opened_at'] : null,
    half_open_at: typeof row['half_open_at'] === 'string' ? row['half_open_at'] : null,
    last_failure_at: typeof row['last_failure_at'] === 'string' ? row['last_failure_at'] : null,
...
export async function main(rawInput: unknown): Promise<{
  ...
  try {
    ...
    switch (action) {
      ...
      case 'record_failure': {
        const errorMessage = (rawInput as Record<string, unknown>)['error_message'] as string | undefined;
