...
def auto_fix_common_mypy_errors():
    ...
    for filepath in files:
        try:
            ...
            content = re.sub(r'def\s+([a-zA-Z0-9_]+)\s*\(([^)]*)\)\s*:$', r'def \1(\2) -> None:', content, flags=re.MULTILINE)
            content = re.sub(r'async\s+def\s+([a-zA-Z0-9_]+)\s*\(([^)]*)\)\s*:$', r'async def \1(\2) -> None:', content, flags=re.MULTILINE)
